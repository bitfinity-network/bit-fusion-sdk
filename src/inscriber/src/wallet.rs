...
impl CanisterWallet {
    ...
    pub async fn inscribe(
        ...
        let commit_tx_result = match inscription_type {
            Protocol::Brc20 => {
                ...
                self.build_commit_transaction::<Brc20>(
                    &mut builder,
                    inscription,
                    dst_address.clone(),
                    leftovers_address,
            ...
            Protocol::Nft => {
                ...
                self.build_commit_transaction::<Nft>(
                    &mut builder,
                    inscription,
                    dst_address.clone(),
                    leftovers_address,
        ...
        let reveal_tx =
            Self::build_reveal_transaction(&mut builder, commit_tx_result.clone(), dst_address)
                .await?;

        print("Sending commit transaction...");
        bitcoin_api::send_transaction(self.bitcoin_network, serialize(&commit_tx_result.tx)).await;
        print("Done");
        ...
        T: Inscription + DeserializeOwned,
    {
        ...
        let commit_tx_result = builder
            .build_commit_transaction(self.network, recipient_address.clone(), commit_tx_args)
            .await?;
