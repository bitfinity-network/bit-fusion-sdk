...
#[tokio::test]
async fn test_many_equal_btc_to_erc20_orders_should_have_unique_nonces() {
    let ckbtc = CkBtcSetup::new().await;
    ...
    let mut utxos = Vec::with_capacity(64);

    for _ in 0..utxos.capacity() {
        let deposit_value = 100_000_000;
        let utxo = Utxo {
            height: 12,
            outpoint: OutPoint {
        ...
        assert_eq!(
            ...
            Ok(Erc20MintStatus::Scheduled {
                ...
                pending_utxos: Some(vec![PendingUtxo {
                    outpoint: btc_bridge::ck_btc_interface::OutPoint {
                        txid: btc_bridge::ck_btc_interface::Txid::try_from(
    ...
    let deposit_value: u64 = utxos.iter().map(|utxo| utxo.value).sum();
    ...
    let total_fee = (ckbtc.kyt_fee() - CKBTC_LEDGER_FEE) * utxos.len() as u64;

    let expected_balance = (deposit_value - total_fee) as u128;
    let balance = (&ckbtc.context)
        .check_erc20_balance(&ckbtc.wrapped_token, &wallet, None)
        .await
        .unwrap();
