...
async fn inputs_from_different_users() {
    ...
    let ctx = Arc::new(ctx);
    ctx.withdraw(&recipient, &rune_id, 50)
        .await
...
#[tokio::test]
async fn test_rune_bridge_stress_test() {
    let ctx = DfxTestContext::new(&CanisterType::RUNE_CANISTER_SET).await;

    let config = StressTestConfig {
        users_number: 5,
        user_deposits_per_token: 1,
        init_user_balance: 100u64.into(),
    ...
    crate::context::stress::rune::stress_test_rune_bridge_with_ctx(ctx, 1, config).await;
}
