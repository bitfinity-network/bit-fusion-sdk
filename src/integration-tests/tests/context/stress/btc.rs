...
impl<'a> UserWallet<'a> {
    async fn new(wallet: Wallet<'a, SigningKey>) -> Self {
...
pub struct BtcToken<'a, Ctx>
where
    Ctx: TestContext + Sync,
{
    ctx: Arc<BtcContext<Ctx>>,
    max_supply: Amount,
    users: AsyncMap<H160, Arc<UserWallet<'a>>>,
    token_ids: Vec<Id256>,
}

impl<Ctx> BtcToken<'_, Ctx>
where
    Ctx: TestContext + Sync,
{
    ...
    async fn user_wallet(&self, user: &H160) -> Result<Arc<UserWallet<'_>>> {
        self.users
            .read()
            .await
            .get(user)
...
impl<Ctx> BaseTokens for BtcToken<'_, Ctx>
where
    Ctx: TestContext + Send + Sync,
{
    ...
    async fn deposit(
        ...
        let amount = info.amount.0.as_u64();
        if self.max_supply.to_sat() < amount {
            return Err(TestError::Generic("Mint amount exceeds max supply".into()));
        }
