...
impl RunesContext<DfxTestContext> {
    /// Init Rune context for DFX tests
    pub async fn dfx(runes: &[String]) -> Self {
    ...
    pub async fn send_runes(
        ...
        for (rune_id, _) in runes {
            ...
            for outpoint in balance.keys() {
                let outpoint_info = OrdClient::dfx_test_client().get_outpoint(outpoint).await?;
