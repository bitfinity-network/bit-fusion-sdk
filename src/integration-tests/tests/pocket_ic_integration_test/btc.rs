...
use super::{PocketIcTestContext, MANAGEMENT_CANISTER_KEY_ID};
...
async fn ledger_wasm() -> Vec<u8> {
    get_icrc1_token_canister_bytecode().await
}

async fn minter_wasm() -> Vec<u8> {
    get_ck_btc_minter_canister_bytecode().await
}

async fn bitcoin_mock_wasm() -> Vec<u8> {
    get_btc_canister_bytecode().await
}

async fn kyt_wasm() -> Vec<u8> {
    get_kyt_canister_bytecode().await
}

async fn install_ledger(env: &PocketIcTestContext) -> CanisterId {
    let args = LedgerArgument::Init(
        LedgerInitArgsBuilder::for_tests()
            .with_transfer_fee(0_u8)
            .build(),
    );
    let ledger_canister_principal = env.canisters().icrc1_ledger();
    env.reinstall_canister(ledger_canister_principal, ledger_wasm().await, (args,)).await
        .unwrap();
    CanisterId::unchecked_from_principal(ledger_canister_principal.into())
}
...
async fn install_minter(env: &PocketIcTestContext, ledger_id: CanisterId) -> CanisterId {
    let args: CkbtcMinterInitArgs = CkbtcMinterInitArgs {
        btc_network: ic_ckbtc_minter::lifecycle::init::BtcNetwork::Regtest,
        ecdsa_key_name: MANAGEMENT_CANISTER_KEY_ID.to_string(),
    ...
    let minter_canister_principal = env.canisters().ck_btc_minter();
    env.reinstall_canister(minter_canister_principal, minter_wasm().await, (minter_arg,)).await
        .unwrap();
    CanisterId::unchecked_from_principal(minter_canister_principal.into())
}
...
#[tokio::test]
async fn test_install_ckbtc_minter_canister() {
    let env =  PocketIcTestContext::new(&[CanisterType::Icrc1Ledger, CanisterType::CkBtcMinter, CanisterType::Kyt]).await;
    let ledger_id = install_ledger(&env).await;
    install_minter(&env, ledger_id).await;
}
...
//         for _ in 0..15 {
//             (&self.context).advance_time(Duration::from_secs(10)).await;
...
//     let minted = ckbtc.mint_wrapped_btc(deposit_value, &wallet).await;
...
//     assert_eq!(minted as u128, balance);
...
//     (&ckbtc.context)
//         .advance_time(Duration::from_millis(4200))
//         .await;
//     ckbtc.burn_btc_to(&wallet, &address, minted).await;

//     (&ckbtc.context).advance_time(Duration::from_secs(10)).await;

//     let txid = ckbtc.await_btc_transaction(3, 10).await;
...
//         (deposit_value - (ckbtc.kyt_fee() * tx_count) - CKBTC_LEDGER_FEE) as u128;
//     let balance = (&ckbtc.context)
//         .check_erc20_balance(&ckbtc.wrapped_token, &wallet, None)
//         .await
//         .expect("Failed to check balance");
