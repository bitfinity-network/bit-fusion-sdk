...
IID=$(docker exec -it ord-cli ord --regtest --bitcoin-rpc-url bitcoind:18443 wallet --server-url http://ord:1338 inscribe --fee-rate 1 --file /brc20_json_inscriptions/brc20_transfer.json --destination $WALLET_ADDRESS | jq -r ".inscriptions[0].id")
docker exec -it bitcoind bitcoin-cli -rpcuser=user -rpcpassword=pass -regtest generatetoaddress 1 $WALLET_ADDRESS
docker exec -it ord-cli ord --regtest --bitcoin-rpc-url bitcoind:18443 wallet --server-url http://ord:1338 send --fee-rate 1 $TO_ADDRESS $IID
docker exec -it bitcoind bitcoin-cli -rpcuser=user -rpcpassword=pass -regtest generatetoaddress 1 $WALLET_ADDRESS
