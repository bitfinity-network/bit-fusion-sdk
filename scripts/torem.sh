...
NFT_TICKER="kobp"
nft_inscription="{\"content_type\": \"text/plain\", \"body\":\"demo\"}"

echo "Creating a NFT inscription with content: $nft_inscription"
TXS=$(dfx canister call btc-nft-bridge inscribe "(variant { Nft }, \"${nft_inscription//\"/\\\"}\", \"${BRIDGE_ADDRESS}\", \"${BRIDGE_ADDRESS}\", null)")
...
for i in 1 2 3; do
  sleep 5
  echo "Trying to bridge from NFT to ERC721"
  mint_status=$(dfx canister call btc-nft-bridge nft_to_erc721 "(\"$NFT_TICKER\", \"$BRIDGE_ADDRESS\", \"$ETH_WALLET_ADDRESS\")")
