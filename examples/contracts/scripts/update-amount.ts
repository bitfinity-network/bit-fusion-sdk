...
async function main() {
  ...
  const ownerManager = new NonceManager(owner);

  const senderManager = new NonceManager(sender);
  ...
  await ownerManager.sendTransaction({
    nonce: await ownerManager.getTransactionCount(),
    ...approve_tx,
  });
